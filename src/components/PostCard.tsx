...
export const PostCard: React.FC<{ post: IPost } & React.HTMLAttributes<HTMLDivElement>> = ({ post, className, style, ...props }) => {
  return (
    <div
      className={`bg-blue-400 hover:bg-blue-300 flex flex-col gap-y-3 my-3 p-4 shadow-2xl rounded-lg transition duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-lg ${className}`}
      style={style}
      {...props}  // Spread additional props like event handlers
    >
      <div className="font-semibold text-2xl truncate text-white">
